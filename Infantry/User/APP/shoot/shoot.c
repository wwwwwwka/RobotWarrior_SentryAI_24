...
int16_t shoot_control_loop(void)
{
    ...
    else if(shoot_control.shoot_mode == SHOOT_READY)
    {
        ...
        buzzer_off();
    } else if(shoot_control.shoot_mode == SHOOT_BARREL_CHANGE)
    {
        ...
        if(shoot_control.barrel == 0 &&shoot_control.heat_1 == 0 && ReadPos(2)/100 == 22)
        {
            shoot_control.barrel=1;
        }else if(shoot_control.barrel == 1 && shoot_control.heat_0 == 0 && ReadPos(2)/100==14 ){
            shoot_control.barrel=0;
        }

        if(barrel_cnt >= 700 && (ReadPos(2)/100==14||ReadPos(2)/100 == 22)){
            barrel_cnt = 0;
            change_barrel(shoot_control.barrel);
        }

        /*if(barrel_cnt >= 700){
            barrel_cnt = 0;
            if(shoot_control.barrel == 0 &&shoot_control.heat_1 == 0 && (ReadPos(2)/100==14||ReadPos(2)/100 == 22))
            {
                shoot_control.barrel=1;
                change_barrel(1);

            }else if(shoot_control.barrel == 1 && shoot_control.heat_0 == 0 && (ReadPos(2)/100==14||ReadPos(2)/100 == 22)){
                shoot_control.barrel=0;
                change_barrel(1);
            }
        }*/
...
static void shoot_set_mode(void)
{
    ...
    if(shoot_control.shoot_mode > SHOOT_BARREL_CHANGE)
    {
    ...
    //printf("%d\n",ReadPos(2));
    if(((shoot_control.heat_0 + 60 > shoot_control.heat_limit_0)||(shoot_control.heat_1 + 60 > shoot_control.heat_limit_1))&&shoot_control.shoot_mode != SHOOT_BARREL_CHANGE)
    {
        shoot_control.shoot_mode = SHOOT_BARREL_CHANGE;

    }else if(shoot_control.shoot_mode == SHOOT_BARREL_CHANGE){
    
        if(shoot_control.barrel == 0 && ReadPos(2)/100==22)//对应枪管为左枪管
        {
           shoot_control.shoot_mode = SHOOT_READY;
        }else if(shoot_control.barrel == 1 && ReadPos(2)/100 == 14)//对应枪管为右枪管
        {
           shoot_control.shoot_mode = SHOOT_READY;
        }
    }
